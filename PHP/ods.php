...
class ods {
	...
	function array2ods() {
		...
		foreach ($sheetArray as $tableIndex => $tableContent) {
			...
			$string .= '<table:table-column table:style-name="co1" table:number-columns-repeated="1" table:default-cell-style-name="Default"/>';
			$string .= '<table:table-column table:style-name="co2" table:number-columns-repeated="4" table:default-cell-style-name="Default"/>';
			...
			foreach ($tableContent['rows'] as $rowIndex => $rowContent) {
				...
				foreach($rowContent as $cellIndex => $cellContent) {
					...
					if (isset($cellContent['bold'])) {
						if ($cellContent['bold']) {
							$string .= 'table:style-name="cebold" ';
						}
					}
					...
					if (isset($cellContent['value'])) {
						if (substr ( $cellContent['value'], 0, 4) === "http") {
							$string .= '<text:p><text:a xlink:href="'.$cellContent['value'].'" xlink:type="simple">'.$cellContent['value'].'</text:a></text:p>';
						} else {
